...
ISR(TIMER1_COMPA_vect) {
  ...
  // Each sample generates a single PWM pulse.  The output is set now, and then
  // cleared by the output match.  A bias is added to the signal to prevent
  // underflow, since the timer has already advanced from zero.  The scaling of
  // values here is hardcoded, but should be tied to the sample rate.  I.e., at
  // 22.05kHz output on a 16MHz Arduino, the timer divisor is 725, so choosing a
  // pulse width of 128 to 640 clocks works, but it won't for faster sampling
  // rates.
  OCR1C = ((dh<<1)|(dl>>7)) + 128; // nine-bit PWM plus bias
#endif
...
void WaveHC::play(void) {
  ...
  // Set up timer one
#if !ROMIBO_AUDIO_PWM_ENABLED
  TCCR1A = 0;                        // Normal operation - no pwm not connected to pins
  TCCR1B = _BV(WGM12) | _BV(CS10);   // no prescaling, CTC mode
#else
  // If audio PWM is enabled, OC1C is used in 16-bit Fast PWM mode.
  const uint8_t CS   =  1;   // prescaler divider is 1 (timer runs off system clock)
  const uint8_t WGM  = 15;   // see ATmega2560 manual Table 17-2, mode 15, 16-bit Fast PWM with rate determined by OCRnA
  const uint8_t IC   =  0;   // no input capture or filter
  const uint8_t COMA =  0;   // no PWM on OCnA
  const uint8_t COMB =  0;   // no PWM on OCnB
  const uint8_t COMC =  2;   // audio PWM OCnC, set output at bottom count, clear at match
  TCCR1A = (COMA << 7) | (COMB << 4) | (COMC << 2) | (WGM & 3); 
  TCCR1B = (IC << 6) | ((WGM & 0x0c) << 1) | (CS); 
  OCR1C = 256+128;  // mid-range on a 9-bit PWM output
#endif
...
#if ROMIBO_AUDIO_PWM_ENABLED
  // Clear the audio PWM output to turn the driver off.
  TCCR1A &= ~(3<<2);     // disconnect OC1C output match from pin
  AUDIO_PWM_PORT &= ~_BV(AUDIO_PWM_BIT);  // set output low
#endif
